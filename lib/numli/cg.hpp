...
struct diremitter : item
{
  ...
  float specular;
  constexpr float Radiance(float l, ℝ3 const &o, hitinfo const &hinfo) const
    { return radiance(l)*std::pow(max(o|hinfo.n(),0.f),specular); }
};
...
/// @todo finish quaddirlight probs
  ...
  RNG &rng)
{
  ...
  info.val = r.normalized();

  // compute prob by converting from sr at hit to area at light
  float const dist2 = r|r;
  float const cosθ = -r|bra::column<3>(qdl._plane.T.M,2).normalized();
  info.prob = 
    (qdl.spec+1)*.5f*inv_π<float>*std::pow(cosθ,qdl.spec-1)*qdl.inv_area*dist2;
